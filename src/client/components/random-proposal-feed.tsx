...
export function RandomProposalFeed({
  ...
  useEffect(() => {
    if (!active || !buffer?.suggestions.length || !sentinel) return;
  ...
  return (
    <div hidden={!active}>
      <ProposalPages
        ...
        footer={
          <div ref={setSentinel} className="empty" aria-live="polite">
            {error ? (
              <>
                <p>{error}</p>
                <button
              ...
              <section aria-label="Next steps">
                ...
                {exhausted ? (
                  <>
                    <p>
                      Random samples cover your selected districts and City-wide projects. The
                      catalog also lets you browse outside that selection.
                    </p>
                    <p>
                      The Completionist achievement counts proposals actually viewed across all
                      districts. Open the catalog to explore the rest, including cards you
                      previously skipped.
                    </p>
                  </>
                ) : (
