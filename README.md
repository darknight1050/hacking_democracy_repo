...
**Review & checkout** loads only the account's funded projects, initially sorted by allocation. Compact add/remove-vote controls let users return coins from one project and spend them on another. Removed rows stay available for easy undo during that review. Confirmation requires 1–100 coins, so users need not exhaust the wallet. Existing confirmed batches are imported into the basket and can be rebalanced too. A revised allocation replaces the previous one only when confirmed; previous votes continue counting while edits remain drafts. Funding may be revised until voting closes. Hidden/deleted projects must be removed before a new confirmation.
