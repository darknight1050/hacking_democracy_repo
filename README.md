...
Application results preserve all core MES winners, then apply greedy 0/1 knapsack completion: rank remaining supported projects by confirmed votes per CHF, breaking ties by total support, cost and ID. Skip unaffordable projects and keep funding until no supported project fits. This is not exact knapsack optimization, and an unavoidable remainder can remain. Projects with no confirmed support are not funded. The old winning-rank count does not apply. Winner cards show MES winners first and greedy additions afterwards; the full ranking remains ordered by raw votes. Individual voter profiles stay server-side. Historical simulation scenarios retain their separately documented rules.
...
The shared suggestion form and owner/admin editor support an optional location label and map pin. Users can tap the map, drag the pin, or enter coordinates with the keyboard. City-wide projects may use a meeting/collection point or omit coordinates. Coordinates do not automatically change the district. Both coordinate values must be supplied together and be in range; explicit blanks clear a stored pin, while omitted location fields preserve values for older edit clients. Creation now requires an explicit cost rather than assuming CHF 10,000. Editing shows the current image, and save confirmations distinguish published, pending and hidden suggestions using the server response.
...
Map uses Leaflet with OpenStreetMap tiles and clustered proposal pins. All matching locations are loaded through `/api/suggestions/map`, independently of card pagination, with only IDs, titles and coordinates. Clusters split on zoom and overlapping pins fan out when selected at maximum zoom. Individual details are fetched on demand through the approved-only public project endpoint. Migration 013 stores latitude/longitude; all 50 Zürich fixtures have approximate demo coordinates. City-wide pins are illustrative touring hubs. Projects without coordinates remain in the list. Tile failures show an explicit message while pins remain usable.
